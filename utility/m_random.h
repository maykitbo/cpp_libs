...
template <class Type>
struct Easy<Type,
    std::enable_if_t<std::is_integral<Type>::value && !std::is_same<Type, bool>::value>> {
    static Type R(Type min, Type max) {
        return Int<Type>(min, max);
    }
};

// Specialization for all floating-point types
template <typename Type>
struct Easy<Type, std::enable_if_t<std::is_floating_point<Type>::value>> {
    static Type R(Type min, Type max) {
        return Uniform<Type>(min, max);
    }
};
