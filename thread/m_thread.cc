...
ThreadManager::ThreadManager(unsigned thread_count)
...
void ThreadManager::Run() {
  ...
  {
    std::unique_lock<std::mutex> lock(mtx_);
    cv_.wait(lock, [&] { return finished_ == ready_.size(); });
  }
  finished_ = 0;
}
...
std::function<void(int)> ThreadManager::LoopThreads(
    unsigned thread_count, int loop_k, const std::function<void(int)> &func) {
  return [=, &func](int thc) {
