...
class Matrix {
    public:
        ...
        inline static double fp_precision = 1e-6;

        Matrix() = default;
        Matrix(i_type rows, i_type cols, T value = T());
        Matrix(i_type rows);
        Matrix(i_type rows, i_type cols, const data_type &data);
        Matrix(i_type rows, i_type cols, data_type &&data);
        ...
        const data_type &DataVector() const;
...
template<class T>
Matrix<T>::Matrix(i_type rows, i_type cols, const data_type &data) :
        rows_(rows), cols_(cols), data_(data) {}

template<class T>
Matrix<T>::Matrix(i_type rows, i_type cols, data_type &&data) :
        rows_(rows), cols_(cols), data_(data) {}
...
template<class T>
void Matrix<T>::Fill(const data_type &data) {
    data_ = data;
}

template<class T>
void Matrix<T>::Fill(data_type &&data) {
    data_ = data;
}
...
bool Matrix<T>::operator==(const Matrix &other) const {
    ...
    if constexpr (std::is_floating_point_v<T>) {
        for (i_type i = 0; i < rows_; ++i) {
            for (i_type j = 0; j < cols_; ++j) {
                if (std::abs(data_[cols_ * i + j] - other.data_[cols_ * i + j]) > fp_precision) {
                    return false;
                }
            }
        }
...
template<class T>
typename Matrix<T>::data_type &Matrix<T>::DataVector() {
    return data_;
}

template<class T>
const typename Matrix<T>::data_type &Matrix<T>::DataVector() const {
    return data_;
}
