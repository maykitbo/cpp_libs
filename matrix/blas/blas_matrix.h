...
class Matrix {
 ...
 public:
  ...
  Matrix(size_t row, size_t col, T value)
      : rows_(row), cols_(col), matrix_(row * col, value) {}

  Matrix(size_t row, size_t col, const std::vector<T> &vec)
      : rows_(row), cols_(col), matrix_(vec) {}
  ...
  Matrix(size_t row, size_t col, std::function<void(T &)> func)
      : Matrix(row, col) {
    for (auto &i : matrix_) {
      func(i);
    }
  ...
  void Update(const std::vector<T> &vec) { matrix_ = vec; }
  ...
  T &operator()(size_t row, size_t col) { return matrix_[cols_ * row + col]; }
 ...
 public:
  ...
  static void Mul(const Matrix<T> &A, const Matrix<T> &B, Matrix<T> &C,
    ...
    func::mul(CblasRowMajor, CblasNoTrans, CblasNoTrans, A.rows_, B.cols_,
              A.cols_, alpha, A.matrix_.data(), A.cols_, B.matrix_.data(),
              B.cols_, beta, C.matrix_.data(), B.cols_);
  }
  static void MulAT(const Matrix<T> &A, const Matrix<T> &B, Matrix<T> &C,
    ...
    func::mul(CblasRowMajor, CblasTrans, CblasNoTrans, A.cols_, B.cols_,
              A.rows_, alpha, A.matrix_.data(), A.cols_, B.matrix_.data(),
              B.cols_, beta, C.matrix_.data(), B.cols_);
  }
  static void MulBT(const Matrix<T> &A, const Matrix<T> &B, Matrix<T> &C,
    ...
    func::mul(CblasRowMajor, CblasNoTrans, CblasTrans, A.rows_, B.rows_,
              A.cols_, alpha, A.matrix_.data(), A.cols_, B.matrix_.data(),
              B.cols_, beta, C.matrix_.data(), B.rows_);
  }
  static void Mul(const std::vector<T> &A, const Matrix<T> &B, Matrix<T> &C,
    ...
    func::mul(CblasRowMajor, CblasNoTrans, CblasNoTrans, 1, B.cols_, B.rows_,
              alpha, A.data(), B.rows_, B.matrix_.data(), B.cols_, beta,
              C.matrix_.data(), B.cols_);
  }
