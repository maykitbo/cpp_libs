...
TEST(compare, constructor) {
  ...
  Compare(CM2, BM2);
  CL::Matrix<long long int> CM3(2, 3,
                                std::vector<long long int>{1, 2, 3, 4, 5, 6});
  BLAS::Matrix<long long int> BM3(2, 3,
                                  std::vector<long long int>{1, 2, 3, 4, 5, 6});
  Compare(CM3, BM3);
}
...
TEST(compare, mul) {
  ...
  CL::Arithmetic::Mul<float>(CL::Matrix<float>{{1, 2, 3, 4}, {5, 6, 7, 8}},
                             CL::Matrix<float>{{11}, {22}, {33}, {44}}, CM);
  BLAS::Matrix<float> BM(2, 1);
  BLAS::Arithmetic<float>::Mul(BLAS::Matrix<float>{{1, 2, 3, 4}, {5, 6, 7, 8}},
                               BLAS::Matrix<float>{{11}, {22}, {33}, {44}}, BM);
  Compare(CM, BM);
}
...
//     // std::cout << (cl_res == s_res) << " " << (s_res == mkl_res) << " " <<
//     (cl_res == mkl_res) << "\n";
//     // for (int k = 0; k < M * N; ++k) {
